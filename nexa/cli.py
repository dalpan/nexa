    ...
    async with HttpClient(
        ...
        if not no_subdomain:
            ...
            if subdomains:
                ...
                # Always crawl top 3 subdomains by default; --crawl-subdomains increases to 10
                sub_limit = 10 if crawl_subdomains else 3
                console.print(f"  Crawling top {min(len(subdomains), sub_limit)} subdomains...")
                for sub in subdomains[:sub_limit]:
        ...
        # 9. Probe sensitive files / debug endpoints
        console.print("[bold cyan]»[/bold cyan] Probing sensitive files & endpoints...")
        probe_findings = await probe_sensitive_files(client, target, domain, concurrency=concurrency)
        result.findings.extend(probe_findings)
        if probe_findings:
            console.print(f"  Found {len(probe_findings)} exposed file(s)")

        # 10. CORS check — test target root + any detected API endpoints
        console.print("[bold cyan]»[/bold cyan] Checking CORS configuration...")
        api_endpoints = list(dict.fromkeys(
            [target] + [
                f.source_url for f in result.findings
                if f.category == Category.INTERNAL_ENDPOINT and f.source_url.startswith("http")
            ]
        ))
        cors_tasks = [check_cors(url, domain) for url in api_endpoints[:5]]
        cors_results = await asyncio.gather(*cors_tasks, return_exceptions=True)
        for cors_r in cors_results:
            if isinstance(cors_r, list):
                result.findings.extend(cors_r)

        # 11. Deduplicate + validate
        result.findings = deduplicate_findings(result.findings)
        console.print(f"  Found {len(result.findings)} unique findings (total)")
